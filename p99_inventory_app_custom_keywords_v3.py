...
def add_name_highlights(ws, keywords, name_col='B'):
    ...
    target = f'A2:{end_col}{ws.max_row}'
...
def style_table_sheet(ws, table_name, keywords):
    for cell in ws[1]:
...
def rebuild_summary(wb, keywords):
    if 'Item Summary' in wb.sheetnames:
        del wb['Item Summary']

    grouped = defaultdict(lambda: {'total': 0, 'files': set()})

    for ws in wb.worksheets:
        if ws.title == 'Item Summary':
            continue
        headers = [ws.cell(1, c).value for c in range(1, min(ws.max_column, 6) + 1)]
        ...
        for r in range(2, ws.max_row + 1):
            ...
            if not name or str(name).strip().lower() == 'empty':
                continue
            item_id = ws.cell(r, 3).value
            count = ws.cell(r, 4).value or 0
            link = ws.cell(r, 6).value or wiki_link(name)
            key = (str(name), item_id, link)
            grouped[key]['total'] += int(count)
            grouped[key]['files'].add(ws.title)
    ...
    rows.sort(key=lambda x: (-x[5], -x[2], x[0]))
    ...
    ws.freeze_panes = 'A2'
    ref = f'A1:{get_column_letter(ws.max_column)}{ws.max_row}'
    tab = Table(displayName=unique_table_name(wb, 'ItemSummaryTable'), ref=ref)
    tab.tableStyleInfo = TABLE_STYLE
    ws.add_table(tab)
...
def import_files(input_files, output_file, log, keywords):
    ...
    for f in input_files:
        rows = parse_inventory_file(f)
        sheet_name = clean_sheet_name(Path(f).stem)

        if sheet_name in wb.sheetnames:
            ws = wb[sheet_name]
            clear_sheet(ws)
            log(f'Updated existing sheet: {sheet_name}')
        else:
            ws = wb.create_sheet(sheet_name)
            log(f'Created new sheet: {sheet_name}')

        ws.append(['Location', 'Name', 'ID', 'Count', 'Slots', 'Wiki Link'])
        for row in rows:
            ws.append(row)

        style_table_sheet(ws, unique_table_name(wb, 'Table'), keywords)
        log(f'Imported {Path(f).name} -> sheet {sheet_name}')
...
class App:
    def __init__(self, root):
        ...
        self.logbox = tk.Text(frm, height=12, wrap='word')
    ...
    def remove_keyword(self):
        sel = list(self.keyword_list.curselection())
        sel.reverse()
        for i in sel:
