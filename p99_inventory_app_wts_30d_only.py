...
def fetch_price_data(url, log):
    ...
    for row in data:
        ...
        prices[name.lower()] = {
            'item_id': row.get('i', ''),
            'type': type_name,
            'wts_30d_avg': row.get('a30', '') if type_name == 'WTS' else '',
...
def apply_price_columns(ws, prices, name_col=2, start_col=7):
    headers = ['PigParse Link', 'WTS 30d Avg']
    ...
    for r in range(2, ws.max_row + 1):
        ...
        if not p:
            continue
        vals = [p['pigparse_link'], p.get('wts_30d_avg', '')]
...
def format_inventory_sheet(ws, wb, keywords):
    ...
    widths = {1:24, 2:42, 3:12, 4:12, 5:12, 6:55, 7:45, 8:14}
...
def rebuild_summary(wb, keywords, prices):
    ...
    ws.append(['Name', 'ID', 'Total Count', 'Files Found In', 'Wiki Link', 'PigParse Link', 'WTB 30d Avg'])
    for row in rows:
        name = row[0]
        p = prices.get(str(name).strip().lower(), {})
        ws.append(row[:5] + [p.get('pigparse_link', ''), p.get('wts_30d_avg', '')])
    format_header(ws)
    for r in range(2, ws.max_row + 1):
