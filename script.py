...
script = base / 'p99_inventory_app_final.py'
script.write_text(r'''import re
import threading
import traceback
from pathlib import Path
...
build_console = base / 'build_console_exe.bat'
build_console.write_text(r'''@echo off
cd /d "%~dp0"
...
build_windowed = base / 'build_windowed_exe.bat'
build_windowed.write_text(r'''@echo off
cd /d "%~dp0"
py -m pip install pyinstaller openpyxl
...
run_py = base / 'Run-P99-Inventory-App-Final.bat'
run_py.write_text(r'''@echo off
cd /d "%~dp0"
...
readme = base / 'README-P99-Final.txt'
readme.write_text(r'''Files included:
- p99_inventory_app_final.py : final Python app
- Run-P99-Inventory-App-Final.bat : runs the Python app directly
- build_console_exe.bat : builds a console EXE for debugging
- build_windowed_exe.bat : builds a normal windowed EXE

Recommended build order:
1. Run build_console_exe.bat
2. Test the EXE in the dist folder from PowerShell with:
   .\p99_inventory_app_final.exe
3. If it works, run build_windowed_exe.bat

Requirements on Windows:
- Python installed
- py launcher available
- Internet access for pip install, or packages already installed
''', encoding='utf-8')
print('created')